...
struct PitLocationSection: View {
    ...
    ///
    /// Only one tab is on screen at a time, so this never runs alongside that
    /// poll — but it is still a second caller against the same Cloudflare budget
    /// `CLAUDE.md` flags as tight at two teams. Queuing statuses turn over on the
    /// order of minutes, so half the rate costs nothing you'd notice on a map.
    ...
    var body: some View {
        ...
        .task(id: eventKey) {
            ...
            // Cancelled when the tab goes away, so this stops the moment you
            // switch tabs rather than polling behind the Schedule tab's own.
    ...
    private func addressList(_ addresses: [PitAddress]) -> some View {
        VStack(spacing: 0) {
            ForEach(Array(addresses.enumerated()), id: \.element.team) { index, entry in
                HStack(spacing: 8) {
                    // A dot rather than recolouring the row: the team's own
                    // highlight colour already owns the text, and two colours on
                    // one string reads as a rendering bug.
                    Circle()
        ...
        LazyVGrid(
            ...
            ForEach(teams, id: \.self) { team in
                ...
                VStack(spacing: 1) {
                    ...
                    if let status {
                        // Wraps rather than shrinks. A shrunk label is unreadable
                        // at caption2 and the cell has vertical room to spare.
                        Text(status.label)
                            .font(.caption2)
                            .foregroundStyle(status.color)
                    ...
                    .padding(.vertical, 8)
                    .background {
                        // Fill for "mine", outline for "what's happening" —
                        // the same two channels the map uses, so the two
    ...
    private func load() async {
        ...
        // Paint from the cache first so the map isn't colourless for a round
        // trip. `loadCachedSchedule` deliberately doesn't check which event it
        // holds, so that check belongs here.
